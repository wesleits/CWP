...
// Estrutura do WPA 4-way handshake
typedef struct _wpapsk_cap
{
	uint8_t essid[32];
	uint8_t sessid;
	uint8_t smac[6];      // supplicant/station MAC
	uint8_t snonce[32];   // supplicant/station nonce
	uint8_t amac[6];      // authenticator/AP MAC
	uint8_t anonce[32];   // authenticator/AP nonce
	uint8_t keyver;       // key version
	uint8_t keymic[16];   // EAPOL frame MIC (2nd key frame)
	uint8_t eapol_size;   // EAPOL frame size (2nd key frame)
	uint8_t eapol[192];   // EAPOL frame contents (2nd key frame)
} wpapsk_cap;
